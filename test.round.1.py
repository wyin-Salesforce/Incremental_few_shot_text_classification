...
class RteProcessor(DataProcessor):
    ...
    def load_Round_1_train(self, filename):
        '''
        classes: ["entailment", "neutral", "contradiction"]
        '''
    ...
    def load_Round_1_dev_or_test(self, filename, base_class2example_list, class_2_ood):
        '''
        classes: ["entailment", "neutral", "contradiction"]
        '''
...
def main():
    ...
    round_1_train_examples, round_1_class2example_list = processor.load_Round_1_train('/export/home/Dataset/incrementalFewShotTextClassification/round1_train.txt')

    base_class_id_set = set()
    for class_name in base_class2example_list.keys():
        base_class_id_set.add(class_list.index(class_2_ood.get(class_name)))

    round_1_new_class_id_set = set()
    for class_name in round_1_class2example_list.keys():
        round_1_new_class_id_set.add(class_list.index(class_2_ood.get(class_name)))


    known_truncate_example_size = 0

    known_selected_class2example_list = {}
    for class_i, ex_list in base_class2example_list.items():
        truncate_ex_list = random.sample(ex_list, 5)
        known_selected_class2example_list[class_i] = truncate_ex_list
        known_truncate_example_size+=len(truncate_ex_list)
    for class_i, ex_list in round_1_class2example_list.items():
        truncate_ex_list = ex_list#random.sample(ex_list, 5)
        known_selected_class2example_list[class_i] = truncate_ex_list
        known_truncate_example_size+=len(truncate_ex_list)
    ...
    round_1_test_examples = processor.load_Round_1_dev_or_test('/export/home/Dataset/incrementalFewShotTextClassification/round1_test.txt', known_selected_class2example_list, class_2_ood)
    label_list = ["pos", "neg"]

    num_labels = len(label_list)
    print('num_labels:', num_labels, 'testing size:', len(round_1_test_examples))#, 'base dev size:', len(base_dev_examples), 'base test size:', len(base_test_examples))

    model = RobertaForSequenceClassification(num_labels)
    tokenizer = RobertaTokenizer.from_pretrained(pretrain_model_dir, do_lower_case=args.do_lower_case)
    model.load_state_dict(torch.load('/export/home/Dataset/BERT_pretrained_mine/MNLI_pretrained/incremental.round.1.pt'), strict=False)
    model.to(device)
    ...
    if args.do_train:
        ...
        # base_dev_dataloader = examples_to_features(base_dev_examples, label_list, class_list, args, tokenizer, args.eval_batch_size, "classification", dataloader_mode='sequential')
        round_1_test_dataloader = examples_to_features(round_1_test_examples, label_list, class_list, args, tokenizer, args.eval_batch_size, "classification", dataloader_mode='sequential')
        ...
        logger.info("  Num examples = %d", len(round_1_test_examples))
        ...
        # print('Evaluating...')
        for _, batch in enumerate(tqdm(round_1_test_dataloader, desc="evaluating")):
        ...
        assert len(pos_probs)%known_truncate_example_size == 0
        test_instance_size = len(pos_probs)//known_truncate_example_size
        assert test_instance_size ==  len(round_1_test_examples)//known_truncate_example_size

        '''for each test example'''
        # for each_test_id in range(test_instance_size):


        hit_co = [0,0,0] #base, round1, ood
        total_co = [0,0,0]
        for each_test_id in range(test_instance_size):
            gold_class_set = set(all_premise_class_ids[each_test_id*known_truncate_example_size: (each_test_id+1)*known_truncate_example_size])
            assert len(gold_class_set) == 1
            gold_class = list(gold_class_set)[0] # can be in the base or ood

            sub_pos_probs = pos_probs[each_test_id*known_truncate_example_size: (each_test_id+1)*known_truncate_example_size]
            sub_hypo_class_ids = all_hypo_class_ids[each_test_id*known_truncate_example_size: (each_test_id+1)*known_truncate_example_size]

            hypo_class_2_problist = {}
            for i in range(known_truncate_example_size):
            ...
            if gold_class == class_list.index('ood'):
                ...
                if final_pred_class == gold_class:
                    hit_co[2]+=1
            elif gold_class in base_class_id_set:
                total_co[0]+=1
                if final_pred_class == gold_class:
                    hit_co[0]+=1
            elif gold_class in round_1_new_class_id_set:
                total_co[1]+=1
                if final_pred_class == gold_class:
                    hit_co[1]+=1
            else:
                print('wrong gold class')
                exit(0)
...
CUDA_VISIBLE_DEVICES=7 python -u test.round.1.py --task_name rte --do_train --do_lower_case --num_train_epochs 3 --train_batch_size 20 --eval_batch_size 64 --learning_rate 1e-6 --max_seq_length 128 --seed 42
