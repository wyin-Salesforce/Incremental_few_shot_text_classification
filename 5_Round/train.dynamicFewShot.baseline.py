...
class RteProcessor(DataProcessor):
    ...
    def load_base_support_examples(self, class_list, fake_novel_support_size):
        ...
        filename = '/export/home/Dataset/incrementalFewShotTextClassification/Incremental-few-shot-text-classification-master/dataset/banking77/split/base/train.txt'
        readfile = codecs.open(filename, 'r', 'utf-8')
        for row in readfile:
            parts = row.strip().split('\t')
            assert len(parts)==2
...
CUDA_VISIBLE_DEVICES=7 python -u train.dynamicFewShot.baseline.py --task_name rte --do_train --do_lower_case --num_train_epochs 1 --train_batch_size 20 --eval_batch_size 64 --learning_rate 1e-6 --max_seq_length 16 --seed 42 --round_name 'r1'
